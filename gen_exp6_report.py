...
def fill_content(table):
    ...
    add_three_line_table(cell,
        ...
        rows=[
            ...
            ['1.4', '实体层', '编写Student.java', '10', '10', '肖辉京'],
            ['1.5', '工具层', '编写Dbconn.java', '10', '10', '肖辉京'],
            ['1.6', '模型层', '编写StudentModel.java', '20', '20', '韩志鑫'],
            ['1.7', '控制层', '编写6个Servlet控制器', '30', '30', '韩志鑫'],
            ['1.8', '视图层', '编写4个JSP页面+CSS样式', '30', '30', '肖辉京'],
            ...
            ['2.2', '增量开发', 'CSV导入功能测试', '5', '5', '肖辉京'],
            ['3.1', 'GitHub', '创建仓库并上传项目源码', '10', '10', '韩志鑫'],
            ['4.1', 'UML建模', '分析并绘制类图', '15', '15', '韩志鑫'],
            ['5.1', 'UML建模', '分析并绘制用例图', '10', '10', '肖辉京'],
            ['6.1', 'WBS', '编制WBS及耗时估算', '15', '15', '韩志鑫'],
            ['7.1', '文档', '截图整理与过程陈述', '20', '20', '肖辉京'],
            ['7.2', '文档', '撰写实验总结', '15', '15', '肖辉京'],
            ['7.3', '文档', '生成报告文档', '10', '10', '韩志鑫'],
        ],
        col_widths_cm=[1.2, 2, 5.5, 1.5, 1.5, 2.8]
    )
    add_text_paragraph(cell, '合计预估耗时：315分钟；合计实际耗时：315分钟。小组成员分工：韩志鑫承担约60%的任务量，肖辉京承担约40%的任务量。')
...
def fill_summary(table):
    ...
    add_text_paragraph(cell, '在本次实验中，我（韩志鑫）承担了约60%的任务量，主要负责以下工作：')
